                ...
                workbook.SheetNames.forEach(sheetName => {
                            ...
                            jsonData.slice(1).forEach(row => {
                                ...
                                if (name) {
                                    mergedData.items.push({
                                        ...
                                        parsedDate: (dateIdx !== -1 ? normalizeDate(row[dateIdx]) : null) || effectiveDate
                                    });
                                }
                            });
                        }
                    ...
                    // --- TYPE 3: SALES SUMMARY (Invoicewise) ---
                        ...
                        jsonData.forEach((row, index) => {
                            ...
                            // Check for Date row (e.g. "Date : 01-Nov-2025")
                            if (typeof particulars === 'string' && particulars.includes('Date :')) {
                            ...
                            // Check for Transaction row
                            else if ((currentDate || effectiveDate) && row[7] !== undefined) {
                                ...
                                if (!isNaN(amount) && amount > 0) {
                                    mergedData.transactions.push({
                                        parsedDate: currentDate || effectiveDate,
                                        parsedAmount: amount,
                                        parsedType: 'Sales',
                    ...
                    // --- TYPE 4: PURCHASES ---
                        ...
                        if (dateIdx !== -1 && amountIdx !== -1) {
                            jsonData.slice(1).forEach(row => {
                                ...
                                if (!isNaN(amount) && amount > 0) {
                                    mergedData.transactions.push({
                                        parsedDate: normalizeDate(row[dateIdx]),
                                        parsedAmount: amount,
                                        parsedType: 'Expense',
