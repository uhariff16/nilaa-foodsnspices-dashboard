...
async function run() { 
  const {data} = await supabase.from('transactions').select('*').ilike('customer_name', '%JAYAKODI%').eq('payment_mode', 'Supplier_Payment'); 
  ...
  console.log("Jayakodi Payments Unique after App.jsx dedup:", unique.length);
  console.log("Total Paid Amount after dedup:", unique.reduce((sum, d) => sum + d.amount, 0));
} 
run();
