...
async function main() {
    ...
    const customers = {};
    aprilSales.forEach(t => {
        const c = t.customer_name || 'NULL';
        if (!customers[c]) {
            customers[c] = { revenue: 0, count: 0, items: new Set() };
        }
        customers[c].revenue += parseFloat(t.amount || 0);
        customers[c].count += 1;
        customers[c].items.add(t.item_name);
    });

    console.log("April Sales Grouped by Customer (Sorted by Revenue):");
    const sorted = Object.entries(customers).sort((a, b) => b[1].revenue - a[1].revenue);
    console.table(sorted.slice(0, 15).map(([name, info]) => ({
        customer: name,
        revenue: Math.round(info.revenue),
        count: info.count,
        items: Array.from(info.items).join(', ')
    })));
}
