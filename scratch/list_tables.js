...
async function main() {
    ...
    const { data: testStats, error: errStats } = await supabase.from('customer_stats').select('*').limit(1);
    console.log("Stats schema test columns:", Object.keys(testStats?.[0] || {}), errStats);
}
