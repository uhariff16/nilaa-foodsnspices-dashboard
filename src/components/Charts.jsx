...
const CustomizableChart = ({ title, data, defaultType = 'area', series, yAxisPrefix = '₹' }) => {
  ...
  const renderChart = () => {
    ...
    const renderSeries = (invisible = false) => series.map((s, idx) => {
      ...
      if (type === 'area') {
        return (
          <Area
            ...
            type="monotone"
            dataKey={s.key}
            stroke={invisible ? 'none' : `url(#${gradientId})`}
            fill={invisible ? 'none' : `url(#${gradientId})`}
            fillOpacity={invisible ? 0 : 0.3}
            strokeWidth={2}
          />
        );
      } else if (type === 'bar') {
        return (
          <Bar key={s.key} dataKey={s.key} fill={s.color} radius={[4, 4, 0, 0]} fillOpacity={opacity}>
    ...
    return (
      <div style={{ flex: 1, width: '100%', maxWidth: '100%', minHeight: 0, display: 'flex' }}>

        {/* Frozen Left Side: Y-Axis Only */}
        <div style={{ width: `${yAxisWidth}px`, flexShrink: 0, height: `${chartHeight}px`, overflow: 'hidden' }}>
          <ChartComponent
            width={yAxisWidth}
            height={chartHeight}
            data={data} // Pass data so domain is calculated
            margin={{ top: 10, right: 0, left: 0, bottom: 0 }}
          >
            {/* Invisible Series to ensure Y-Axis Scale matches the real chart */}
            {renderSeries(true)}
            <YAxis
              width={yAxisWidth}
              stroke="#94a3b8"
              ...
              tickLine={false}
              axisLine={false}
              tickFormatter={(val) => `${yAxisPrefix}${new Intl.NumberFormat('en-IN', { notation: "compact", compactDisplay: "short" }).format(val)}`}
            />
          </ChartComponent>
        </div>

        {/* Scrollable Right Side: Data & X-Axis */}
