...
const Dashboard = (props) => {
    ...
    const { logout, user, role, isAdmin, canAccessAttendance, permissions, hasPermission } = useAuth();
    const navigate = useNavigate();
    // Default to Overview tab (per user request)
    // Default to a tab the user actually has permission for
    const [activeTab, setActiveTab] = useState(() => {
        if (isAdmin) return 'overview';
        if (permissions?.overview) return 'overview';
        // Find first available permission
        const firstAvailable = [
            'overview', 'sales', 'expenses', 'procurement', 'stock',
            'production', 'customers', 'simulator', 'attendance'
        ].find(p => permissions?.[p]);
        return firstAvailable || 'overview';
    });

    // Auto-switch to accessible tab if current one is hidden (e.g. after refresh or login)
    useEffect(() => {
        if (!isAdmin && !permissions[activeTab] && activeTab !== 'overview') {
            const firstAvailable = [
                'overview', 'sales', 'expenses', 'procurement', 'stock',
                'production', 'customers', 'simulator', 'attendance'
            ].find(p => permissions?.[p]);
                ...
                {hasPermission('simulator') && (
                    <button
                        ...
                        style={{
                            ...
                            display: 'flex', alignItems: 'center', gap: '0.5rem',
                            boxShadow: activeTab === 'simulator' ? '0 4px 12px rgba(59, 130, 246, 0.3)' : 'none',
